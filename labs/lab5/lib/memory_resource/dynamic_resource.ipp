...
void DynamicResource::do_deallocate(void* p, std::size_t bytes,
                                    std::size_t aligment) {
...
bool DynamicResource::do_is_equal(
...
DynamicResource::~DynamicResource() noexcept {
    for (auto& memory_data : allocated_) {
        ::operator delete(memory_data.data, memory_data.bytes,
                          std::align_val_t(memory_data.aligment));
    }
