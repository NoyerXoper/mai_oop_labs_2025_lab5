...
// unique_ptr: I need to store end, but Queue doesn't own the end directly (the
// last owns it) shared_ptr is not efficient for simple class like Queue on
// dynamic structures raw pointers: easy to operate here, no need to specify the
// deleter for neither unique ptr nor shared ptr. Also are quite efficient.
...
private:
    ...
    public:
        ...
        inline bool
        operator==(const IteratorTemplate<isOtherConst>& other) const noexcept {
