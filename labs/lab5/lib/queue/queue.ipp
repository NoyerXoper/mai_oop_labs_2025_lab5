...
Queue<T, Allocator>::IteratorTemplate<isConst>::IteratorTemplate(Node* node): cur_(node) {}

template<class T, class Allocator>
template<bool isConst>
Queue<T, Allocator>::IteratorTemplate<isConst>::IteratorTemplate(): IteratorTemplate(nullptr) {}


template<class T, class Alloc>
template<bool isConst>
Queue<T, Alloc>::IteratorTemplate<isConst>& Queue<T, Alloc>::IteratorTemplate<isConst>::operator++() noexcept {
    cur_ = cur_->next;
    return *this;
}
...
template<class T, class Alloc>
template<bool isConst>
Queue<T, Alloc>::IteratorTemplate<isConst> Queue<T, Alloc>::IteratorTemplate<isConst>::operator++(int) noexcept {
...
template<class T, class Alloc>
template<bool isConst>
Queue<T, Alloc>::IteratorTemplate<isConst>::reference_type Queue<T, Alloc>::IteratorTemplate<isConst>::operator*() const noexcept {
    return cur_->val;
}

template<class T, class Alloc>
template<bool isConst>
Queue<T, Alloc>::IteratorTemplate<isConst>::pointer Queue<T, Alloc>::IteratorTemplate<isConst>::operator->() const noexcept {
    return std::addressof(cur_->val);
}

template<class T, class Alloc>
template<bool isConst>
Queue<T, Alloc>::IteratorTemplate<isConst>::operator IteratorTemplate<true>() const noexcept {
    return IteratorTemplate<true>(cur_);
}

template<class T, class Alloc>
Queue<T, Alloc>::Queue(const Alloc& alloc): allocator_(alloc), head_(nullptr), end_(nullptr), size_(0) {}

template<class T, class Alloc>
Queue<T, Alloc>::Queue(const std::initializer_list<T>& list, const Alloc& alloc): Queue(list.begin(), list.end(), alloc) {}
...
template<class T, class Alloc>
Queue<T, Alloc>::Queue(const Queue& other): Queue(other.begin(), other.end(), std::allocator_traits<NodeAlloc>::select_on_container_copy_construction(other.allocator_)) {}

template<class T, class Alloc>
Queue<T, Alloc>::Queue(Queue&& other) noexcept: allocator_(std::move(other.allocator_)), head_(other.head_), end_(other.end_), size_(other.size_) {
    other.head_ = nullptr;
    other.end_ = nullptr;
    other.size_ = 0;
}
